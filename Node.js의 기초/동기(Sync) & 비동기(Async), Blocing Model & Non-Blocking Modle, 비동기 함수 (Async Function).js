...
// 2. Non-Blocking Model : 코드의 실행이 끝나지 않아도 실행 제어권을 다른곳에 넘겨 다음 코드가 실행될 수 있는것을 말한다.

// 자바 스크립트는  Async + Non-Blocking Model을 채용하여 현재 실행중인 코드의 실행이 끝나지 않아도 다음 코드를 호출한다.
...
//  첫 번째 함수resolve는 비동기 작업을 성공적으로 완료해 결과를 값으로 반환할 때 호출해야 하고, 두 번째 함수reject는 작업이 실패하여 오류의 원인을 반환할 때 호출하면 됩니다. 두 번째 함수는 주로 오류 객체를 받는다.
...
// 일반적으로 객체Object를 생성하는 함수를 생성자Constructor 함수라고 부르게 되는데, Promise 또한 객체로 구성되어 있기 때문에 생성자 함수를 이용해 Promise를 선언하게 된다.
...
// 이렇게 `executor` 가 실행되어 `resovle`된 프로미스를 Fulfilled Promise라고도 부릅니다.
...
const firstPromise1 = Promise.resolve('First');
...
const firstPromise2 = Promise.resolve('First');
